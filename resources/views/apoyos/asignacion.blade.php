    ...
    <div class="modal fade" id="confirmarModal" tabindex="-1" role="dialog" aria-labelledby="confirmarModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form method="POST" action="{{ route('apoyos.asignacion-crea', ['id_remesa' => $id_remesa, 'cve_ciudad' => $cve_ciudad, 'tipo_partida' => 2]) }}">
                        @csrf
                        <button type="submit" class="btn btn-verde">Crear Nueva Partida</button>
                    </form>
                    <form method="POST" action="{{ route('apoyos.asignacion-crea', ['id_remesa' => $id_remesa, 'cve_ciudad' => $cve_ciudad, 'tipo_partida' => 3]) }}">
                        @csrf
    ...
    <div class="card mx-auto">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('apoyos.asignacion', [$id_remesa, $cve_ciudad]) }}">
                <div class="row">
                    <label for="id_remesa" class="col-md-1 col-form-label text-md-right">{{ __('Remesa') }} </label>
                    <div class="col-md-4">
                        <select id="id_remesa" name="id_remesa" class="form-control" @error('id_remesa') is-invalid @enderror aria-label="Default select example" onchange="this.form.submit()">
                            <option value='-1'>-- SELECCIONA REMESA --</option>
                    ...
                    <label for="cve_ciudad" class="col-md-1 col-form-label text-md-right">{{ __('Ciudad') }} </label>
                    <div class="col-md-3">
                    ...
                    <label for="id_partida" class="col-md-1 col-form-label text-md-right">{{ __('Partida') }} </label>
                    <div class="col-md-2">
                                ...
                                @foreach ($estudiantes as $estudiante)
                                    ...
                                    <tr style="font-size:13px">
                                        ...
                                        @if ($id_remesa != 0 && $cve_ciudad != 0)
                                            @php
                                                $apoyos = $estudiante->apoyosAsignados()
                                                        ->where('id_remesa', $id_remesa)
                                                        ->first();
                                                $id_partida = is_null($apoyos) ? '' : $apoyos->id_partida . ' :: ';

                                                $monto_asignado = HelperFunctions::monto_asignado($id_remesa, $estudiante->id);

                                                if ($monto_asignado == 'N/A') $estudiantes_sin_apoyos++;
                                            @endphp
                                            {{ $id_partida . $monto_asignado  }}
                                        @endif
                                        &nbsp;</td>
                                        @if ($usertype == 1)
                                        <td style="vertical-align:middle">
